...
if kubectl get secret signing-secrets -n tekton-chains >/dev/null 2>&1; then
  KEY_COUNT=$(kubectl get secret signing-secrets -n tekton-chains -o json | \
    python3 -c "import sys,json; print(len(json.load(sys.stdin).get('data',{})))" 2>/dev/null || echo "0")
  if [ "${KEY_COUNT}" -gt 0 ]; then
    check_pass "Cosign signing secret exists (${KEY_COUNT} keys)"
  else
    check_fail "Cosign signing secret exists but is EMPTY — run: kubectl delete secret signing-secrets -n tekton-chains && COSIGN_PASSWORD='' cosign generate-key-pair k8s://tekton-chains/signing-secrets"
  fi
else
